...
*/
    ...
    private fun startRide() {
       ...
       /*     val progressDialog = ProgressDialog(this@TravelDashboard)
            ...
            var mAPIService: ApiService? = null
            mAPIService = ApiClient.apiService
            mAPIService!!.rideStart("Bearer " + sessionManager.fetchAuthToken(),  sessionManager.fetchuserid()!!,sessionManager.fetchrequestid()!!).enqueue(object :
                Callback<startRideResponse> {
                override fun onResponse(call: Call<startRideResponse>, response: Response<startRideResponse>) {
                    Log.d("", "post submitted to startride API." + response.body()!!)
                    Log.d("startride_post" , response.body().toString()!!)
                    if (response.isSuccessful()) {
                        var lo: startRideResponse = response.body()!!
                        if (lo.status == 200) {
                           // progressDialog.dismiss()
                           ...
                           // progressDialog.dismiss()
                            ...
                            Toast.makeText(this@TravelDashboard, "Ride Is not started", Toast.LENGTH_SHORT).show()
                        }

                    }
                }

                override fun onFailure(call: Call<startRideResponse>, t: Throwable) {
                    Toast.makeText(this@TravelDashboard, t.message, Toast.LENGTH_SHORT).show()
                }
            })
        }catch (e:java.lang.Exception){
        ...
        try {
            ...
            sessionManager.fetchuserid()?.let {
                mAPIService!!.online(
                    ...
                    override fun onResponse(call: Call<backgroundcheck>, response: Response<backgroundcheck>) {
                        Log.d("", "post submitted to Online API." + response.body()!!)
                        if (response.isSuccessful()) {
                            Log.d("", " Online API Response." + response.toString()!!)
                            var lo: backgroundcheck = response.body()!!
                            if (lo.status == 200) {
                                availabilty = "Online"
                                sessionManager.saveAvailabity(availabilty!!)
    ...
    private fun offlineapi() {
       ...
       val progressDialog = ProgressDialog(this@TravelDashboard)
        ...
        sessionManager.fetchuserid()?.let {
            ...
            override fun onResponse(call: Call<backgroundcheck>, response: Response<backgroundcheck>) {
                Log.d("", "post submitted to Offline API." + call.request().toString())
                if (response.isSuccessful()) {
                    var lo: backgroundcheck = response.body()!!

                    Log.d("", " Offline API Response." + response.toString()!!)
